...
namespace SpaceGame
{
    ...
    /// C3 "Testudo" is the tortoise super-heavy carrier: a huge scute-
    /// plated shell dome over the hold, a beaked tortoise head on a neck
    /// out front, four rounded leg pods (the rear pair doubling as
    /// engines), amber rim lights around the shell skirt, and a stub tail.
    /// Submeshes: 0 sand, 1 olive, 2 amber glow, 3 dark glass.
    /// Streams: packbody / packpanels (C1), pack2body / pack2panels (C2),
    /// pack3body / pack3panels (C3).
    /// </summary>
    public static class PackShipMesh
    {
        ...
        static GameObject BuildC1(string hash, Transform shipRoot)
        {
            ...
            for (int hump = 0; hump < 2; hump++)
            {
                ...
                float hw = g.HumpW * W;
                float hh = g.HumpH * H;
                BevelBox(b, new Vector3(0f, y0 + hh * 0.28f, z), new Vector3(hw * 0.55f, hh * 0.30f, 0.72f), 0);
                BevelBox(b, new Vector3(0f, y0 + hh * 0.62f, z - 0.04f), new Vector3(hw * 0.42f, hh * 0.24f, 0.56f), 0);
                BevelBox(b, new Vector3(0f, y0 + hh * 0.92f, z - 0.08f), new Vector3(hw * 0.28f, hh * 0.18f, 0.40f), 1);
                BevelBox(b, new Vector3(0f, y0 + hh * 1.12f, z - 0.08f), new Vector3(0.10f, 0.03f, 0.12f), 2);
                for (int side = -1; side <= 1; side += 2)
                {
                    BevelBox(b, new Vector3(side * hw * 0.56f, y0 + hh * 0.30f, z),
            ...
            for (int side = -1; side <= 1; side += 2)
            {
                for (int leg = 0; leg < 3; leg++)
                {
                    ...
                    BevelBox(b, foot + new Vector3(0f, -0.03f, 0f), new Vector3(0.06f, 0.028f, 0.16f), 1);
                }
            }
            ...
            // ---- one modest dorsal gun ahead of the first hump ----
            {
                ...
                Fairing(b, mount + new Vector3(0f, -0.08f, 0f), Vector3.up, 0.18f, 0.24f, 0.045f, 10, 1);
                BevelBox(b, mount, new Vector3(0.10f, 0.06f, 0.14f), 1);
            ...
            for (int side = -1; side <= 1; side += 2)
            {
                for (int i = 0; i < 6; i++)
                {
                    ...
                    BevelBox(b, c, new Vector3(0.02f, 0.03f, 0.08f), 2);
                }
            }
            ...
            // ---- twin boxy engines with amber exhaust ----
            {
                ...
                for (int side = -1; side <= 1; side += 2)
                {
                    ...
                    BevelBox(b, ec, new Vector3(0.24f, 0.22f, 0.48f), 1);
                    BevelBox(b, ec + new Vector3(0f, 0.05f, 0.28f), new Vector3(0.16f, 0.13f, 0.16f), 0);
                    Nozzle(b, ec + new Vector3(0f, 0f, -0.55f), Vector3.back, 0.185f, 0.26f, 10, 1, 2);
                    int n = g.EngineSegs;
                    for (int i = 0; i < n; i++)
        ...
        static GameObject BuildC2(string hash, Transform shipRoot)
        {
            ...
            // ---- segmented loading trunk stepping down off the face ----
            {
                ...
                for (int i = 0; i < g.TrunkSegs; i++)
                {
                    ...
                    BevelBox(b, c, new Vector3(w2, 0.055f, 0.13f), i % 2 == 0 ? 0 : 1);
                    if (i % 2 == 1)
                        BevelBox(b, c + new Vector3(0f, 0.06f, 0f), new Vector3(w2 * 0.55f, 0.012f, 0.09f), 2);
                }
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                float z = zAt(t);
                float y0 = hullY(0, t);
                BevelBox(b, new Vector3(0f, y0 + 0.24f * H, z), new Vector3(W * 0.60f, H * 0.26f, 0.58f), i % 2 == 0 ? 0 : 1);
                BevelBox(b, new Vector3(0f, y0 + 0.24f * H, z - 0.60f), new Vector3(W * 0.62f, H * 0.20f, 0.03f), 1);
                BevelBox(b, new Vector3(0f, y0 + 0.50f * H, z), new Vector3(0.08f, 0.02f, 0.10f), 2);
            }
            for (int side = -1; side <= 1; side += 2)
            {
                for (int i = 0; i < 3; i++)
            ...
            for (int side = -1; side <= 1; side += 2)
            {
                for (int leg = 0; leg < 2; leg++)
                {
                    ...
                    float z = zAt(t);
                    float ls = g.LegScale;
                    BevelBox(b, new Vector3(side * W * 0.72f, -H * 0.62f * ls, z),
                        new Vector3(0.19f, H * 0.26f * ls, 0.26f), 1);
                    BevelBox(b, new Vector3(side * W * 0.75f, -H * 1.02f * ls, z - 0.02f),
                        new Vector3(0.15f, H * 0.22f * ls, 0.21f), 0);
                    BevelBox(b, new Vector3(side * W * 0.75f, -H * 1.26f * ls, z),
                        new Vector3(0.20f, H * 0.06f * ls, 0.30f), 1);
                    BevelBox(b, new Vector3(side * W * 0.75f, -H * 0.82f * ls, z + 0.20f),
            ...
            // ---- one modest dorsal gun on the crown ----
            {
                ...
                float y0 = hullY(0, t0);
                var mount = new Vector3(0f, y0 + 0.05f, zAt(t0));
                Fairing(b, mount + new Vector3(0f, -0.08f, 0f), Vector3.up, 0.19f, 0.25f, 0.045f, 10, 1);
                BevelBox(b, mount, new Vector3(0.11f, 0.06f, 0.15f), 1);
                var gb = mount + new Vector3(0f, 0.08f, 0.08f);
                var mz = gb + new Vector3(0f, 0.015f, g.GunLen);
            ...
            // ---- wide four-engine drive block ----
            {
                ...
                for (int side = -1; side <= 1; side += 2)
                    for (int row = 0; row < 2; row++)
                    {
                        ...
                        BevelBox(b, ec, new Vector3(0.22f, 0.19f, 0.44f), row == 0 ? 1 : 0);
                        Nozzle(b, ec + new Vector3(0f, 0f, -0.51f), Vector3.back, 0.17f, 0.24f, 10, 1, 2);
                    }
                int n = g.EngineSegs;
                for (int i = 0; i < n; i++)
        ...
        {
            public float L, W, H;
            public float DomeH, HeadSize, PodR, PodLen;
            public float GunLen, TailLen;
            public int EngineSegs;
            public float Hue, Sat, Val, OliveHue, MarkOdds, SurfAmp, SurfPhase;
            public int BandMode, BandCount;
            public float BandPhase;
        ...
        static GenomeP3 RollP3(string hash)
        {
            var rng = Rng.Stream("pack3body:" + hash);
            System.Func<float, float, float> R = (lo, hi) => lo + (float)rng.NextDouble() * (hi - lo);
            var g = new GenomeP3();
            g.L = R(12.5f, 13.5f);
            g.W = R(2.2f, 2.4f);
            g.H = R(1.3f, 1.45f);
            g.DomeH = R(0.9f, 1.1f);
            g.HeadSize = R(0.9f, 1.1f);
            g.PodR = R(0.26f, 0.32f);
            g.PodLen = R(0.9f, 1.2f);
            g.GunLen = R(1.1f, 1.4f);
            g.TailLen = R(0.5f, 0.7f);
            g.EngineSegs = 3 + rng.Next(2);
            g.Hue = R(0.10f, 0.13f);
            g.Sat = R(0.30f, 0.45f);
            g.Val = R(0.75f, 0.85f);
        ...
        static GameObject BuildC3(string hash, Transform shipRoot)
        {
            ...
            // Tortoise underbody: a wide flat oval that the shell sits on.
            float[] cts = { 0.00f, 0.08f, 0.22f, 0.40f, 0.58f, 0.75f, 0.90f, 1.00f };
            float[] csc = { 0.40f, 0.72f, 0.94f, 1.00f, 1.00f, 0.96f, 0.86f, 0.62f };
            float[] clf = { -0.03f, 0.00f, 0.02f, 0.03f, 0.03f, 0.02f, 0.00f, -0.03f };
            ...
            // ---- the shell: a scute-plated dome over the whole back ----
            {
                const int segs = 16;
                const int rows = 6;
                float yBase = hullY(0, 0.5f) - 0.06f;
                float zC = zAt(0.52f);
                float zR = 0.42f * L;
                float xR = W * 1.12f;
                var dome = new Vector3[rows][];
                for (int ri = 0; ri < rows; ri++)
                {
                    float phi = ri / (float)rows * Mathf.PI * 0.5f;
                    float y = yBase + g.DomeH * Mathf.Sin(phi);
                    float shrink = Mathf.Cos(phi);
                    dome[ri] = new Vector3[segs];
                    for (int k = 0; k < segs; k++)
                    {
                        float ang = k / (float)segs * Mathf.PI * 2f;
                        dome[ri][k] = new Vector3(
                            xR * shrink * Mathf.Cos(ang), y, zC + zR * shrink * Mathf.Sin(ang));
                    }
                }
                var apex = new Vector3(0f, yBase + g.DomeH, zC);
                for (int ri = 0; ri < rows - 1; ri++)
                    for (int k = 0; k < segs; k++)
                    {
                        int k2 = (k + 1) % segs;
                        int mat = ri < 2 ? 1 : 0;
                        b.QuadUDS(dome[ri][k], dome[ri][k2], dome[ri + 1][k2], dome[ri + 1][k], mat);
                    }
                for (int k = 0; k < segs; k++)
                    b.TriUDS(dome[rows - 1][k], dome[rows - 1][(k + 1) % segs], apex, 0);
                // Seam studs on the upper scutes and rim lights on the skirt.
                for (int k = 0; k < segs; k += 2)
                {
                    Ball(b, dome[3][k], 0.045f, k % 4 == 0 ? 2 : 1, 2, 5);
                    Ball(b, dome[0][k] + Vector3.up * 0.03f, 0.04f, 2, 2, 5);
                }
            }

            // ---- beaked tortoise head on a neck out front ----
            {
                float hs = g.HeadSize;
                var neckBase = new Vector3(0f, -0.05f * H, 0.50f * L - 0.10f);
                var headC = new Vector3(0f, -0.02f * H, 0.50f * L + 0.55f * hs);
                Tube(b, new[] { neckBase, headC }, new[] { 0.22f * hs, 0.18f * hs }, 8, 1, false);
                BevelBox(b, headC + Vector3.forward * (0.18f * hs), new Vector3(0.30f * hs, 0.22f * hs, 0.32f * hs), 1);
                BevelBox(b, headC + Vector3.forward * (0.42f * hs) + Vector3.up * (0.02f * hs),
                    new Vector3(0.22f * hs, 0.15f * hs, 0.10f * hs), 0);
                BevelBox(b, headC + Vector3.forward * (0.50f * hs) + Vector3.up * (0.09f * hs),
                    new Vector3(0.15f * hs, 0.045f * hs, 0.03f * hs), 3);
                for (int side = -1; side <= 1; side += 2)
                    BevelBox(b, headC + new Vector3(side * 0.24f * hs, -0.04f * hs, 0.30f * hs),
                        new Vector3(0.02f, 0.03f, 0.06f), 2);
                BevelBox(b, headC + Vector3.forward * (0.52f * hs) - Vector3.up * (0.08f * hs),
                    new Vector3(0.10f * hs, 0.04f * hs, 0.04f * hs), 1);
            }

            // ---- four rounded leg pods; the rear pair are engines ----
            for (int side = -1; side <= 1; side += 2)
            {
                for (int leg = 0; leg < 2; leg++)
                {
                    float t = leg == 0 ? 0.28f : 0.74f;
                    var c = new Vector3(side * W * 0.82f, -0.34f * H, zAt(t));
                    Tube(b, new[] { c + Vector3.forward * (g.PodLen * 0.5f), c - Vector3.forward * (g.PodLen * 0.5f) },
                        new[] { g.PodR, g.PodR }, 8, leg == 0 ? 1 : 0, true);
                    if (leg == 0)
                    {
                        BevelBox(b, c + Vector3.forward * (g.PodLen * 0.5f + 0.03f),
                            new Vector3(g.PodR * 0.6f, g.PodR * 0.5f, 0.025f), 3);
                        BevelBox(b, c + new Vector3(side * g.PodR * 0.9f, 0f, 0.1f),
                            new Vector3(0.012f, 0.04f, 0.14f), 2);
                    }
                    else
                    {
                        Nozzle(b, c - Vector3.forward * (g.PodLen * 0.5f + 0.09f), Vector3.back,
                            g.PodR * 0.9f, g.PodR * 1.2f, 10, 0, 2);
                        BevelBox(b, c + new Vector3(side * g.PodR * 0.9f, 0f, 0f),
                            new Vector3(0.012f, 0.04f, 0.16f), 2);
                    }
                }
            }

            // ---- one modest gun on the shell crown ----
            {
                var mount = new Vector3(0f, hullY(0, 0.5f) - 0.06f + g.DomeH + 0.02f, zAt(0.30f));
                Fairing(b, mount + new Vector3(0f, -0.07f, 0f), Vector3.up, 0.17f, 0.23f, 0.04f, 10, 1);
                BevelBox(b, mount, new Vector3(0.10f, 0.05f, 0.13f), 1);
                var gb = mount + new Vector3(0f, 0.07f, 0.07f);
                var mz = gb + new Vector3(0f, 0.012f, g.GunLen);
                Tube(b, new[] { gb, mz }, new[] { 0.048f, 0.04f }, 7, 1, false);
                Tube(b, new[] { mz, mz + Vector3.forward * 0.08f }, new[] { 0.046f, 0.04f }, 7, 0, true);
            }

            // ---- stub tail and center engine ----
            {
                var basePt = new Vector3(0f, -0.10f * H, -0.50f * L - 0.02f);
                Tube(b, new[] { basePt, basePt + new Vector3(0f, -0.06f, -g.TailLen * 0.6f), basePt + new Vector3(0f, -0.16f, -g.TailLen) },
                    new[] { 0.10f, 0.06f, 0.012f }, 6, 1, true);
                var ec = new Vector3(0f, 0.10f * H, zAt(0.97f));
                Tube(b, new[] { ec + Vector3.forward * 0.25f, ec - Vector3.forward * 0.40f },
                    new[] { 0.20f, 0.20f }, 8, 1, false);
                Nozzle(b, ec - Vector3.forward * 0.49f, Vector3.back, 0.21f, 0.30f, 10, 1, 2);
                int n = g.EngineSegs;
                for (int i = 0; i < n; i++)
                    Tube(b, new[] { ec + new Vector3(0f, 0f, 0.18f - i * 0.18f), ec + new Vector3(0f, 0f, 0.10f - i * 0.18f) },
                        new[] { 0.215f, 0.215f }, 8, i % 2 == 0 ? 0 : 1, false);
            }

            var go = new GameObject("Hull");
            go.transform.SetParent(shipRoot, false);
            var mesh = new Mesh { name = "pack3_" + hash };
            mesh.SetVertices(b.V);
            mesh.subMeshCount = 4;
            for (int m = 0; m < 4; m++) mesh.SetTriangles(b.Sub[m], m);
            mesh.RecalculateNormals();
